...
func (s *server) mergeAndPushToRemote(sourceOrg, sourceRepo, destOrg, destRepo string, sourceRemoteResolver func() (string, error), branch string, dry bool) (string, error) {
	...
	if !merged {
		return "", fmt.Errorf("couldn't merge %s/%s, due to merge conflict: create a new PR in %s/%s which merges/resolves from %s/%s, then use /publicize there to merge all changes into the public repository", destOrg, destRepo, sourceOrg, sourceRepo, destOrg, destRepo)
	}
