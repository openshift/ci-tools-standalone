...
func TestLoadConfig(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expectedError, err, equateErrorMessage); diff != "" {
...
func TestValidatePolicies(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expected, actual, equateErrorMessage); diff != "" {
				t.Errorf("%s differs from expected:\n%s", tc.name, diff)
			}
		})
	}
...
func TestSaveS3BackoffCache(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.expectedError, actualErr, equateErrorMessage); diff != "" {
				t.Errorf("error differs from expected:\n%s", diff)
			}
		})
	}
